...
-- because converting the already exiting column did not work for some reason,
...
-- in the database there a rows with the same "ParcelID" which means the same property is in the database more than once.
...
-- we can take the "PropertyAdress" of the property if it exist in a diffrent "UniqueID" row with same "ParcelID" (which means same property). 
...
-- 'Yes' and 'No' so i updated this rows using case command to make the data in this column in one format.
...
-- using Row_Number function to check for duplicated data we delete rows that are duplicated
...
-- Delting the columns that we dont need anymore after we inserted their data in better way (for analyzing) into new columns
